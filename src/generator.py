...
def output_one():
    '''
    TODO: 真寄吧乱啊，狠狠重构，不重构就艾草
    '''
    t0 = dt.now()

    output_settings = config.CONFIG['output']
    name1 = os.getcwd() + output_settings['image_path'] + "sample_" + time.strftime("%Y-%m-%d_%H-%M-%S")
    name2 = os.getcwd() + output_settings['label_path'] 
    if not os.path.exists(name2):
        os.makedirs(name2)
    name3 = name2 + 'classes.txt'
    name2 += "sample_" + time.strftime("%Y-%m-%d_%H-%M-%S") + '.txt'
    with open(name3, 'w') as f:
        f.write('\n'.join(pfl.LABELS))
    render_one(name1) 
    label_one(name2)

    info(f'渲染耗时{dt.now() - t0}.')
...
def label_one(name):
    ...
    for col in pfl.get_instance_col().children:
        for o_name in col.objects.keys():
            # cx, cy, w, h, dep
            yolo_res = get_bounding_box(o_name)
            if yolo_res is None:
                continue
...
def get_bounding_box(obj_name, cam_name = 'Camera'):
    '''
    @return: []
    '''
